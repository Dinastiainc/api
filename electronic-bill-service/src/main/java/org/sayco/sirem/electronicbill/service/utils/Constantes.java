package org.sayco.sirem.electronicbill.service.utils;

import java.util.Date;

/**
 * clase que almacena las constantes que no vienen desde el request pero se deben guardar en la base de datos
 */

public class Constantes {

    public final static String BODEGA = "BOSAYCO";
    //public final static String ORIGEN = "FAC";
    public final static String MEVERSION = "DIAN_00";
    public final static String TIPOPER = "NN";
    public final static int CANTIDAD = 1;
    public final static String DEFAULT_DATE = "1/01/1900";

}
